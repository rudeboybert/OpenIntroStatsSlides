...
%
%Why the different point estimate for SLR vs multiple?
%
%\pause\vspace{0.5cm}
%
...
%Say for:
%\begin{itemize}
%\pause\item Model 1: $\widehat{y}_i = b_0 + b_1 x_{1,i}$ \ with $e_i = y_i - \widehat{y}_i$
%\pause\item Model 2: $\widehat{y}_i^* = b_0^* + b_1^* x_{1,i} + \blue{b_2^* x_{2,i}}$ \ with $e_i^* = y_i - \widehat{y}_i^*$
%\end{itemize}
%
%\pause both models' fit is near identical, i.e.
%\begin{eqnarray*}
%Var(e_i) \approx Var(e_i^{*}) \ldots
%\end{eqnarray*}
%
%\pause Then pick Model 1 over Model 2 since it is \blue{simpler}: it has fewer of predictors $k$.
