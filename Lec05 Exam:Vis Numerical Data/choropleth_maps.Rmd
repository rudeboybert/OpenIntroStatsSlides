...
This `.Rmd` file is code based on a blog post by Reed College's newest member of the ITS Department Rich Marjerus who specializes in data visualization.  If you have questions about data visualization or are interested in learning more about how to make dynamic maps in R, please stop by his office (ETC 223) or email him at majerus@reed.edu.  

Whenever possible I will use data about Reed in my posts so that we can all learn about our institution in the process. While I was looking around the Reed website I found data on the geographic distribution of the college"s entering classes from 2007-2013 on the [Institutional Research page](http://www.reed.edu/ir/geographic_states.html). Here is a reproduction of the data: 

Before you run this code, you"ll need to install the `googleVis` R package.

```{r, echo=FALSE, warning=FALSE}
# Load googleVis package
suppressPackageStartupMessages(library(googleVis))

# Load data, which was obtained at http://www.reed.edu/ir/geographic_states.html
...
colnames(state) <- 
  c("State", "yr_2007", "yr_2008", "yr_2009", "yr_2010", "yr_2011", "yr_2012", 
    "yr_2013", "log_2013")
...
GeoStates_log <- 
  gvisGeoChart(state, "State", "log_2013",
               options=list(region="US", displayMode="regions", resolution="provinces",
                            width=600, height=400))

# drop logged variable
state$log_2013 <- NULL
...
hist(log(ifelse(state[,8]!=0, state[,8], 1)))
```
