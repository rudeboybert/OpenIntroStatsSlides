...
%i.e. you are repeating the following 1000 times:
%
%\begin{itemize}
...
%\begin{itemize}
%\pause \item describes how these 1000 instances of $\xbar$ behave
...
\begin{itemize}
\pause \item $n=100$. $SE = \frac{\sigma}{\sqrt{n}} = \frac{2}{\sqrt{100}} = 0.2$
\pause \item $n=1000$. $SE = \frac{\sigma}{\sqrt{n}} = \frac{2}{\sqrt{1000}} = 0.0632$.  \blue{Smaller}!
\end{itemize}

\begin{center}
\pause \includegraphics[width=0.8\textwidth]{figure/compare}
\end{center}
\pause Both are ``accurate'', but the estimates on the right are ``more precise.''
...
\blue{Popular question}:  What's up with this ``1000'' instances?  Why would you take 1000 different samples of size $n$? 

\vspace{0.5cm}

\pause\blue{Answer}: No, in practice you would \blue{not} sample repeatedly:  you do this only \blue{once} for the largest $n$ possible.  

\vspace{0.5cm}

\pause Rather the 1000 instances of $\xbar$ is a theoretical exercise to illustrate that $\xbar$'s are random and we characterize its randomness by its sampling distribution and its standard error.  
...
In this example we knew $\sigma$; typically we won't.  \pause However, when
\begin{itemize}
\item $n \geq 30$
